...
Do not dispatch the first release from `publish.yaml` until a maintainer has
confirmed all of these settings.

- On the npm package **Access** page, set the Trusted Publisher to GitHub
  Actions, organization `photostructure`, repository `knex-sqlite`, workflow
  filename `publish.yaml`, and no environment. Enable only **Allow npm stage
  publish**; leave **Allow npm publish** disabled.
- Set npm publishing access to **Require two-factor authentication and disallow
  tokens**, and confirm that no publish-capable npm token is available to this
  repository.
- In the GitHub Actions settings, select read-only default workflow permissions,
  leave Actions pull-request creation and approval disabled, and require
  approval for workflows from all external contributors.
- Confirm organization 2FA, immutable GitHub releases, and that applicable
  branch and tag rules permit the workflow's signed atomic push. Until a
  narrowly scoped GitHub App is available, the repository `GITHUB_TOKEN` is the
  temporary release identity; the signed commit and tag, exact-SHA checks, and
  staged 2FA approval are its compensating controls.

When updating GitHub Action pins, use pinact 4.0 or later and run
`pinact run --check --verify`; earlier versions ignore `.pinact.yaml`'s
`min_age` policy.

## Prepare the release

1. Add the release notes to `CHANGELOG.md` on `main`. The heading, release date,
   and tag link must match the version bump you will select. Version 1.0.0 is an
   intentional stability release, so select `major` for that prepared entry.
...
validates the tag, packs and verifies the exact tarball, stages it on npm, and
